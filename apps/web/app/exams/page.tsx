  ...
  return (
    <Shell title="Exam Archive" subtitle="Find past exams and practice by subject and year.">
      <div className="space-y-6">
        <form onSubmit={applyFilters} className="card grid gap-3 bg-white/70 p-4 sm:grid-cols-3 sm:p-5">
        ...
        {canCreate ? (
          <form onSubmit={onCreateExam} className="card grid gap-3 bg-white/70 bg-blue-500/10 p-4 sm:grid-cols-3 sm:p-5">
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-dashed border-slate-200 bg-white/60 p-8 text-center text-sm text-slate-500">No exams found for the current filters.</div>
        ) : (
            ...
            {exams.map((exam) => (
              <li key={exam.id} className="card p-4 sm:p-5">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <span className="rounded-full bg-slate-200 px-2 py-1 text-xs text-slate-500">{exam.year}</span>
                </div>
