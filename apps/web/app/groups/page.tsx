  ...
  return (
    <Shell title="Study Groups" subtitle="Create or join focused learning communities.">
      <div className="space-y-6">
        <form onSubmit={onCreateGroup} className="card grid gap-3 bg-white/70 p-4 sm:p-5">
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-dashed border-slate-200 bg-white/60 p-8 text-center text-sm text-slate-500">No groups yet. Create one to start collaborating.</div>
        ) : (
            ...
            {groups.map((group) => (
              <li key={group.id} className="card p-4 sm:p-5">
                <div className="mb-2 flex items-center justify-between gap-2">
                  ...
                  <span className="rounded-full bg-slate-200 px-2 py-1 text-xs text-slate-500">{group.isPrivate ? 'Private' : 'Public'}</span>
                </div>
                <p className="mb-3 text-sm text-slate-500">{group.description || 'No description provided.'}</p>
